...
kgsl_find_context(struct kgsl_device_private *dev_priv, uint32_t id)
{
	...
	if (ctxt && ctxt->dev_priv == dev_priv) {
		return ctxt;
	} else {
		if (ctxt == NULL)
			printk("kgsl warning: kgsl_find_context(), ctxt is NULL, id=%d", id);
		else if (ctxt->dev_priv != dev_priv)
			printk("kgsl warning: kgsl_find_context(), ctxt->dev_priv != dev_priv, ctxt->dev_priv=%p, dev_priv=%p, id=%d",ctxt->dev_priv, dev_priv, id);
		return NULL;
	}
}
