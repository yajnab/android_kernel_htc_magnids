...
static int __init msm_serial_probe(struct platform_device *pdev)
{
	...
	if (unlikely(IS_ERR(msm_port->clk))) {
		printk(KERN_INFO"[msm_serial.%d]error getting core_clk\n", pdev->id);
	...
	resource = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	if (unlikely(!resource)) {
		printk(KERN_INFO"[msm_serial.%d]error getting resource\n", pdev->id);
		return -ENXIO;
	}
	...
	if (unlikely(irq < 0)) {
		printk(KERN_INFO"[msm_serial.%d]error getting irq\n", pdev->id);
		return -ENXIO;
	}
	port->irq = irq;
