...
void brcm_msm_hs_request_clock_on(struct uart_port *uport)
{
	...
	case MSM_HS_CLK_REQUEST_OFF:
		/*msm_uport->request_clk_off_delay = 100;*/ /* bt for brcm */
...
static int msm_hs_startup(struct uart_port *uport)
{
	...
	if (use_low_power_wakeup(msm_uport)) {
		msm_uport->host_wakeup_level = 1; /* bt for brcm */
		msm_uport->rx.is_brcm_rx_wake_locked = 0; /* bt for brcm */
...
#ifdef USE_BCM_BT_CHIP
	msm_uport->clk_off_delay = ktime_set(0, 100000000);  /* 100ms */
#else
