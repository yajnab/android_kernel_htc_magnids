...
static void enable_uart2dm(int);
#endif
...
printk(KERN_INFO "%s():%d\n", __FUNCTION__, __LINE__);
	if (msm_proc_comm == NULL) {
printk(KERN_INFO "msm_proc_comm == NULL\n");
		return;
	}

        msm_proc_comm(PCOM_CUSTOMER_CMD3, &read_cmd, &value);
        printk(KERN_INFO "[GPS] before, value=0x%x\n", value);
	value = 0x10;
        msm_proc_comm(PCOM_CUSTOMER_CMD3, &write_cmd, &value);
        msm_proc_comm(PCOM_CUSTOMER_CMD3, &read_cmd, &value);
        printk(KERN_INFO "[GPS] after, value=0x%x\n", value);
}
