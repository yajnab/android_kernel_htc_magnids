	...
	struct mmc_card *oldcard)
{
	...
	*/
...
#if 0
	/* 2012 March detect write protection status */
	...
	err = mmc_send_write_prot_type(card, WP_STATUS, wp_prevention_start);
	...
	} else if (card->ext_csd.sectors > 4194304) {
		/*  For 2GB eMMC, there will no HC_ERASE_GROUP define */
		printk(KERN_WARNING "%s: ERASE_GROUP_DEF EXT_CSD[175] warning\n",
			mmc_hostname(card->host));
		err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
				 EXT_CSD_ERASE_GROUP_DEF, 1,
				 card->ext_csd.generic_cmd6_time);
