...
/* HTC_START */
		...
		if (s_ctrl->sensordata->use_rawchip) {
			rawchip_data.s_ctrl = s_ctrl;
			rawchip_data.sensor_start_stream = msm_sensor_start_stream;
			rawchip_data.sensor_stop_stream = msm_sensor_stop_stream;
			rawchip_data.sensor_name = s_ctrl->sensordata->sensor_name;
			rawchip_data.datatype = s_ctrl->curr_csic_params->dt;
...
{
	int32_t rc = 0;
	s_ctrl->mode = mode; /* HTC Angie 20120723 */
...
int32_t msm_sensor_set_power_up(struct msm_sensor_ctrl_t *s_ctrl)//(const struct msm_camera_sensor_info *data)
{
	...
	if (data->sensor_reset_enable)
		gpio = data->sensor_reset;
	else
		gpio = data->sensor_pwd;
#ifdef CONFIG_MACH_MAGNIDS
    mdelay(1);
    rc = ioext_gpio_set_value(gpio, 1);
    mdelay(1);
#else
	rc = gpio_request(gpio, "SENSOR_NAME");
	if (!rc) {
		/* HTC_START, for power sequence tuning */
		pr_info("[CAM]%s: reset sensor %d low\n", __func__, gpio);
		...
		pr_err("%s: gpio request fail %d", __func__, gpio);
	}
#endif
	return rc;
}
int32_t msm_sensor_set_power_down(struct msm_sensor_ctrl_t *s_ctrl)//(const struct msm_camera_sensor_info *data)
{
    ...
    {
    int32_t rc = 0;
    rc = ioext_gpio_set_value(gpio, 0);
	return rc;
    }
#else
	/* HTC_START, for power sequence tuning */
	pr_info("[CAM]%s: reset sensor %d high\n", __func__, gpio);
	...
	const struct i2c_device_id *id)
{
	...
	for (hardware_retry = 0; hardware_retry < 3; hardware_retry++) {
		pr_info("hardware_retry=%d\n", hardware_retry);
#ifdef CONFIG_RAWCHIP
		if (s_ctrl->sensordata->use_rawchip) {
#ifdef CONFIG_SPI_CPLD
			/* cpld driver power on */
			pr_info("%s, call cpld_open_init\n", __func__);
			cpld_open_init();
#endif
			rc = rawchip_probe_init();
			if (rc < 0) {
				pr_err("%s rawchip_probe_init failed rc=%d\n", __func__, rc);
#ifdef CONFIG_SPI_CPLD
				/* cpld driver power off */
				pr_info("%s, call cpld_release\n", __func__);
				cpld_release();
#endif
				continue;
			}
...
int32_t msm_sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	mutex_lock(s_ctrl->msm_sensor_mutex);
	if (on) {
/* HTC_START: Add retry mechanism to avoid I2C read error - Move retry to mctl due to shared power */
			rc = s_ctrl->func_tbl->sensor_power_up(s_ctrl);
			if (rc < 0) {
