			...
			struct msm_cam_media_controller *pmctl, void __user *arg)
{
	...
	if (copy_from_user(&dropframe_enabled, arg, sizeof(dropframe_enabled))) {
		...
		pr_info("%s: set dropframe_enabled %d\n", __func__, atomic_read(&pmctl->dropframe_enabled));
			...
			struct msm_cam_media_controller *pmctl, void __user *arg)
{
	...
	if (copy_from_user(&snap_dropframe_num, arg, sizeof(snap_dropframe_num))) {
		...
		pr_info("%s: set snap_dropframe_num %d\n", __func__, atomic_read(&pmctl->snap_dropframe_num));
	}
 ...
 */
static int msm_isp_should_drop_frame(struct msm_cam_media_controller *pmctl,
			uint8_t msgid)
{
	int drop_frame = 0;
...
/* HTC_START chris 20120618 - HDR */
			...
			if (msm_isp_should_drop_frame(pmctl, msgid)) {
				pr_info("[HDR] msm_isp_notify_vfe: output_id %d frame_id %d\n",
					isp_output->output_id, isp_output->frameCounter);
				...
				CDBG("msm_isp_notify_vfe: output_id %d frame_id %d\n",
					isp_output->output_id, isp_output->frameCounter);
				...
				if(msgid<0)
				{
					pr_err(" %s error when msgid = msm_isp_vfe_msg_to_img_mode(pmctl,msgid) \n", __func__);
					return -EINVAL;
				}
				/* HTC_END */
