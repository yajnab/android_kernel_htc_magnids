...
/* HTC_START, function insteaded in s5k4e5yx.act */
#if 0
...
int32_t msm_actuator_af_power_down(struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	if (a_ctrl->step_position_table[a_ctrl->curr_step_pos] !=
		a_ctrl->initial_code) {
		rc = a_ctrl->func_tbl->actuator_set_default_focus(a_ctrl, NULL);
...
int32_t msm_actuator_power_down(struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	/* HTC_START harvey 20120730 */
		...
		rc = ioext_gpio_set_value(a_ctrl->vcm_pwd, 0);
	#else
	...
	struct v4l2_subdev *sdev)
{
	...
	memcpy((void *)(&a_ctrl->sdev), (void *)sdev, sizeof(struct v4l2_subdev));
...
int32_t msm_actuator_power_up(struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	/* HTC_START harvey 20120730 */
	#ifdef CONFIG_MACH_MAGNIDS
		rc = ioext_gpio_set_value(a_ctrl->vcm_pwd, 1);
	#else
...
/* HTC_START, function insteaded in s5k4e5yx_act.c */
#if 0
...
/* HTC_START, initcall insteaded in s5k4e5yx_act.c */
#if 0
