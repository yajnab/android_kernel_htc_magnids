...
msmrtc_timeremote_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	printk("RTC set called from process %s\n", current->comm);
	if (tm->tm_year < 1900)
		tm->tm_year += 1900;
	...
	rc = msm_rpc_client_req(rtc_pdata->rpc_client,
				...
				NULL, NULL, -1);
	printk(KERN_DEBUG "%s: rtc_pdata->rpc_client.prog=0x%x, rtc_pdata->rpc_client.ver=0x%x, rtc_args.proc=%d\n", __func__, rtc_pdata->rpc_client->prog, rtc_pdata->rpc_client->ver, rtc_args.proc);
	if (rc) {
		dev_err(dev, "%s: %.2u/%.2u/%.4u %.2u:%.2u:%.2u (%.2u)\n",
			   __func__, tm->tm_mon, tm->tm_mday, tm->tm_year,
			   tm->tm_hour, tm->tm_min, tm->tm_sec, tm->tm_wday);
		return rc;
	}
...
msmrtc_virtual_alarm_set(struct device *dev, struct rtc_wkalrm *a)
{
	...
	if (now > rtc_pdata->rtcalarm_time) {
		dev_err(dev, "%s: now = %ld, rtcalarm_time = %ld\n",
		       __func__, now, rtc_pdata->rtcalarm_time);
