...
extern unsigned int skip_cpu_offline;
#endif

static void boost_cpufreq_ondemand(int boost)
{
...
static void __ref boost_cpu_hotplug(int boost, int force_down)
{
	static unsigned int boost_target_cpu = 0;
	...
	if (boost) {
		...
		if (num_online_cpus() == 1) {
			boost_target_cpu = 1;
			cpu_up(boost_target_cpu);
		}
	} else {
		skip_cpu_offline = 0;
		if (boost_target_cpu && force_down)
			cpu_down(boost_target_cpu);
		boost_target_cpu = 0;
	}
...
{
	switch (state) {
	case EARLY_SUSPEND_HANDLE_START:
		pr_info("boost cpufreq: start\n");
		boost_cpufreq_ondemand(1);
		break;
	case EARLY_SUSPEND_HANDLE_END:
		boost_cpufreq_ondemand(0);
		pr_info("boost cpufreq: end\n");
		break;
	case LATE_RESUME_HANDLE_START:
		pr_info("boost cpu hotplug: start\n");
		boost_cpu_hotplug(1, 0);
		boost_cpufreq_ondemand(1);
		break;
	case LATE_RESUME_HANDLE_END:
		boost_cpufreq_ondemand(0);
		/* keep 2nd core on for performance */
		boost_cpu_hotplug(0, 0);
		pr_info("boost cpu hotplug: end\n");
		break;
	default:
		return NOTIFY_DONE;
	}
...
#ifdef CONFIG_EARLYSUSPEND_BOOST_CPU_SPEED
	earlysuspend_state_notifier_unregister(&earlysuspend_state_nb);
#endif
}
