...
struct gic_chip_data {
	...
	u32 saved_isr_group[DIV_ROUND_UP(1020, 32)];
...
static void gic_mask_irq(struct irq_data *d)
{
	...
	dummy_r = readl_relaxed(gic_dist_base(d) + GIC_DIST_ENABLE_SET);
...
unsigned int dummy_check[2];
unsigned int *cpu1_foot_print4 = &dummy_check[0];
...
static void gic_eoi_irq(struct irq_data *d)
{
	...
	dummy_r = readl_relaxed(gic_cpu_base(d) + GIC_DIST_ENABLE_SET);
...
static int gic_set_type(struct irq_data *d, unsigned int type)
{
	...
	if (readl_relaxed(base + GIC_DIST_ENABLE_SET + enableoff) & enablemask) {
		...
		dummy_r = readl_relaxed(base + GIC_DIST_ENABLE_SET);
		enabled = true;
	}
...
asmlinkage void __exception_irq_entry gic_handle_irq(struct pt_regs *regs)
{
	...
	do {
		...
		if (irqnr < 16) {
			...
			dummy_r = readl_relaxed(cpu_base + GIC_DIST_ENABLE_SET);
...
static void gic_dist_save(unsigned int gic_nr)
{
	...
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 32); i++) {
		gic_data[gic_nr].saved_isr_group[i] =
			readl_relaxed(dist_base + GIC_DIST_ISR + i * 4);
		//pr_info("ISR saving[%d] 0%x\n", i, gic_data[gic_nr].saved_isr_group[i]);
	}
...
static void gic_dist_restore(unsigned int gic_nr)
{
	...
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 32); i++) {
			writel_relaxed(gic_data[gic_nr].saved_isr_group[i],
					dist_base + GIC_DIST_ISR + i * 4);
		//pr_info("ISR restore [%d] 0%x\n", i, readl_relaxed(dist_base + GIC_DIST_ISR + i * 4));
	}
...
void gic_raise_softirq(const struct cpumask *mask, unsigned int irq)
{
	...
	*(cpu1_foot_print4 + cpu_num) = 0x01;
	 ...
	 */
	dsb();
	*(cpu1_foot_print4 + cpu_num) = 0x02;

#ifdef CONFIG_ARCH_MSM8625
	raw_spin_lock_irqsave(&irq_controller_lock, flags);
#endif
	*(cpu1_foot_print4 + cpu_num) = 0x03;
	/* this always happens on GIC0 */
	writel_relaxed(sgir,
			gic_data_dist_base(&gic_data[0]) + GIC_DIST_SOFTINT);
	dummy_r = readl_relaxed(gic_data_dist_base(&gic_data[0]) + GIC_DIST_ENABLE_SET);
	mb();
	*(cpu1_foot_print4 + cpu_num) = 0x04;
#ifdef CONFIG_ARCH_MSM8625
	raw_spin_unlock_irqrestore(&irq_controller_lock, flags);
#endif
	*(cpu1_foot_print4 + cpu_num) = 0x05;
}
#endif
...
void gic_set_irq_secure(unsigned int irq)
{
	...
	if (is_cpu_secure()) {
		if(d){
			...
			WARN(1, "invaild irq_data=%p while setting gic_set_irq_secure\n",
			d);
		}
	} else {
