...
#if 1
static int oem_rapi_client_cb3(struct msm_rpc_client *client,
...
int oem_rapi_client_streaming_function3(
...
EXPORT_SYMBOL(oem_rapi_client_streaming_function3);

int oem_rapi_client_close3(void)
{
...
EXPORT_SYMBOL(oem_rapi_client_close3);

struct msm_rpc_client *oem_rapi_client_init3(void)
{
	mutex_lock(&oem_rapi_client_lock);
	if (open_count == 0) {
...
EXPORT_SYMBOL(oem_rapi_client_init3);
#endif
static ssize_t htc_keybox_read(struct htc_keybox_dev *dev, char *buf, size_t size, loff_t p)
{
	...
	ret_rpc = oem_rapi_client_streaming_function3(rpc_client, &arg, &ret);
	if (ret_rpc) {
