...
#define MSM_GPIO_BANK(bank, first, last)				\
	{								\
		...
		.chip = {						\
			...
			.get = msm_gpio_get,				\
			.set = msm_gpio_set,				\
			.set_array = msm_gpio_set_array,				\
			.get_array = msm_gpio_get_array,				\
			.direction_input = msm_gpio_direction_input,	\
			.direction_input_array = msm_gpio_direction_input_array,	\
...
/* HTC START */
static int msm_gpio_write_array(struct msm_gpio_chip *msm_chip,
			  unsigned *offset, unsigned *on)
{
	unsigned mask;
	unsigned val;
	unsigned *offsets, *ons;
	int first_gpio_pin = 1;

	val = __raw_readl(msm_chip->regs.out);

	for ( offsets = offset,ons = on; (*offsets || first_gpio_pin); offsets++,ons++ ) {
		first_gpio_pin = 0;
		mask = BIT(*offsets);
		if (*ons == 1)
			val |= (mask);
	...
	__raw_writel(val, msm_chip->regs.out);
...
/* HTC START */
static int msm_gpio_direction_input_array(struct gpio_chip *chip, unsigned *offset)
{
	struct msm_gpio_chip *msm_chip;
	unsigned long irq_flags;
	unsigned val;
	...
	msm_chip = container_of(chip, struct msm_gpio_chip, chip);
	spin_lock_irqsave(&msm_chip->lock, irq_flags);
	val = __raw_readl(msm_chip->regs.oe);
	for ( offsets = offset; *offsets; offsets++ ) {
		val &= ~BIT(*offsets);
	}
	__raw_writel(val, msm_chip->regs.oe);
	mb();
	spin_unlock_irqrestore(&msm_chip->lock, irq_flags);
	return 0;
}
...
/* HTC START */
static void msm_gpio_get_array(struct gpio_chip *chip, unsigned *offset, int *value)
{
	struct msm_gpio_chip *msm_chip;
	unsigned *offsets, *values;
	int first_gpio_pin = 1;

	msm_chip = container_of(chip, struct msm_gpio_chip, chip);

	for ( offsets = offset,values = value; (*offsets || first_gpio_pin); offsets++,values++ ) {
		first_gpio_pin = 0;
		*values = (__raw_readl(msm_chip->regs.in) & (1U << *offsets)) ? 1 : 0;
	}
	mb();
}
/* HTC END */
...
/* HTC START */
static void msm_gpio_set_array(struct gpio_chip *chip, unsigned *offset, int *value)
{
	struct msm_gpio_chip *msm_chip;
	unsigned long irq_flags;

//pr_err("[CAM]  horng test   msm_gpio_set_array()");

	msm_chip = container_of(chip, struct msm_gpio_chip, chip);
	spin_lock_irqsave(&msm_chip->lock, irq_flags);
	msm_gpio_write_array(msm_chip, offset, value);
	mb();
	spin_unlock_irqrestore(&msm_chip->lock, irq_flags);
}
/* HTC END */
