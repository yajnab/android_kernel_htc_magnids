...
static struct perflock_screen_policy screen_policy;

/* Helper macros to lock/unlock screen policy perflock */
#define lock_screen_policy(perflock)		\
if(perflock && !is_perf_lock_active(perflock))	\
{						\
	perf_lock(perflock);			\
}						\

#define unlock_screen_policy(perflock)		\
if(perflock && is_perf_lock_active(perflock))	\
{						\
...
{
	/* screen is switching from ON to OFF */
...
{
	/* screen is switching from OFF to ON */
...
static int earlysuspend_state_notifier(struct notifier_block *this,
					unsigned long state,
					void *data)
{
	switch (state) {
	case LATE_RESUME_HANDLE_START:
		/* Release screen-off lock to prepare for cpufreq boost */
		unlock_screen_policy(screen_policy.off_max);
		break;
	default:
		return NOTIFY_DONE;
...
static struct notifier_block earlysuspend_state_nb = {
	.notifier_call = earlysuspend_state_notifier,
	.priority = 10, //prior to cpufreq_ondemand:boost_cpufreq_ondemand()
};
#endif
...
void __init perflock_screen_policy_init(struct perflock_screen_policy *policy)
{
	if (!policy)
		return;
...
#ifdef CONFIG_EARLYSUSPEND_BOOST_CPU_SPEED
	earlysuspend_state_notifier_register(&earlysuspend_state_nb);
#endif
...
/* 7k projects need to raise up cpu freq before panel resume for stability */
	...
	/* Init perflocks for screen policy. */
	if (policy->on_min != NULL &&
		(policy->on_min->flags & PERF_LOCK_INITIALIZED)) {
		screen_policy.on_min = policy->on_min;
		pr_info("add screen policy perflock: %s\n",
			 screen_policy.on_min->name);
	}
	if (policy->off_max != NULL &&
		(policy->off_max->flags & PERF_LOCK_INITIALIZED)) {
		screen_policy.off_max = policy->off_max;
		pr_info("add screen policy perflock: %s\n",
			 screen_policy.off_max->name);
	}
...
/**
 * perf_lock_init_v2 - acquire a cpufreq_ceiling perf lock
 * @lock: cpufreq_ceiling perf lock to acquire
