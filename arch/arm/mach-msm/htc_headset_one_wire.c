...
static int hs_1wire_init(void)
{
	...
	if (fp->private_data == NULL) {
		...
		if (hi->pdata.ioext_uart_sw)
			ioext_gpio_set_value(hi->pdata.ioext_uart_sw, 0);
#endif
		return -1;
	}
	setup_hs_tty(fp);
...
#ifdef CONFIG_MACH_MAGNIDS
	if (hi->pdata.ioext_uart_sw) {
		ioext_gpio_set_value(hi->pdata.ioext_uart_sw, 1); /* 1: UART I/O to Audio Jack, 0: UART I/O to others */
		HS_LOG("[HS]Set uart sw = 1");
	}
#endif
	hr_msleep(10);
...
static int htc_headset_1wire_probe(struct platform_device *pdev)
{
	...
	hi->pdata.ioext_uart_sw = pdata->ioext_uart_sw;
