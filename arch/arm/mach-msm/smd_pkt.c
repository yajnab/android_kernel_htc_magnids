				  ...
				  size_t n)
{
	...
	if (i >= NUM_SMD_PKT_PORTS) {
		pr_err("[SMD] %s: unable to match device to valid smd_pkt port\n",
			__func__);
		return -EINVAL;
	}
...
wait_for_packet:
	...
	if (r < 0) {
		...
		if (r != -ERESTARTSYS) {
			...
			pr_err("[SMD] %s: wait_event_interruptible on smd_pkt_dev"
			       " id:%d ret %i\n",
...
int smd_pkt_open(struct inode *inode, struct file *file)
{
	...
	if (smd_pkt_devp->ch == 0) {
			...
			** doesn't degrade the performance. */
			...
			if (smd_pkt_devp->open_modem_wait) {
				...
				if (r == 0)
					r = -ETIMEDOUT;
				if (r < 0) {
					pr_err("[SMD] %s: wait on smd_pkt_dev id:%d"
		...
		if (r < 0) {
			pr_err("[SMD] %s: wait on smd_pkt_dev id:%d OPEN event failed"
			       " rc:%d\n", __func__, smd_pkt_devp->i, r);
		} else if (!smd_pkt_devp->is_open) {
			pr_err("[SMD] %s: Invalid OPEN event on smd_pkt_dev id:%d\n",
				__func__, smd_pkt_devp->i);
			r = -ENODEV;
		} else {
			smd_disable_read_intr(smd_pkt_devp->ch);
			...
			r = 0;
			D_STATUS("[SMD] Finished %s on smd_pkt_dev id:%d\n",
				 __func__, smd_pkt_devp->i);
		}
	}
release_pil:
...
static int __init smd_pkt_init(void)
{
	...
	r = alloc_chrdev_region(&smd_pkt_number,
			       ...
			       DEVICE_NAME);
	if (IS_ERR_VALUE(r)) {
		pr_err("[SMD] %s: alloc_chrdev_region() failed ret:%i\n",
		       __func__, r);
		goto error0;
	}
	...
	if (IS_ERR(smd_pkt_classp)) {
		pr_err("[SMD] %s: class_create() failed ENOMEM\n", __func__);
		r = -ENOMEM;
		goto error1;
	}
	...
	for (i = 0; i < NUM_SMD_PKT_PORTS; ++i) {
		...
		if (IS_ERR(smd_pkt_devp[i])) {
			pr_err("[SMD] %s: kzalloc() failed for smd_pkt_dev id:%d\n",
				__func__, i);
			r = -ENOMEM;
			goto error2;
		}
		...
		if (IS_ERR_VALUE(r)) {
			pr_err("[SMD] %s: cdev_add() failed for smd_pkt_dev id:%d"
			       " ret:%i\n", __func__, i, r);
		...
		if (IS_ERR(smd_pkt_devp[i]->devicep)) {
			pr_err("[SMD] %s: device_create() failed for smd_pkt_dev"
			       " id:%d\n", __func__, i);
			r = -ENOMEM;
		...
		if (device_create_file(smd_pkt_devp[i]->devicep,
			...
			pr_err("[SMD] %s: unable to create device attr for"
			       " smd_pkt_dev id:%d\n", __func__, i);
	}

	INIT_DELAYED_WORK(&loopback_work, loopback_probe_worker);

	D_STATUS("[SMD] SMD Packet Port Driver Initialized.\n");
	return 0;
