...
#define ACOUSTIC_SET_VOIP_STATE _IOW(ACOUSTIC_IOCTL_MAGIC, 34, unsigned int)
...
#define ONCRPC_SET_VOIP_STATE_PROC     (16)
	...
	case ACOUSTIC_SET_HEADSET_TYPE:
		...
		if (rc < 0)
			pr_aud_err("ONCRPC_SET_HEADSET_TYPE_PROC failed"
				   " %d.\n", rc);
		break;
	case ACOUSTIC_SET_VOIP_STATE:
		if (copy_from_user(&voip_state, (void *)arg,
			sizeof(voip_state))) {
			rc = -EFAULT;
			break;
		}
		voip_state_req.state = cpu_to_be32(voip_state);
		pr_aud_info("htc_acoustic set_voip_state: %d\n",
			    voip_state);
		rc = msm_rpc_call(endpoint, ONCRPC_SET_VOIP_STATE_PROC,
			&voip_state_req, sizeof(voip_state_req), 5 * HZ);
		if (rc < 0)
			pr_aud_err("ONCRPC_SET_VOIP_STATE_PROC failed"
				   " %d.\n", rc);
		break;
	default:
		rc = -EINVAL;
