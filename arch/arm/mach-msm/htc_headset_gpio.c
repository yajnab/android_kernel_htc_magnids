...
static void irq_init_work_func(struct work_struct *work)
{
	unsigned int hpin_irq_type = IRQF_TRIGGER_LOW;
	HS_DBG();

	if (hi->pdata.hpin_gpio) {
		HS_LOG("Enable detect IRQ");
		if (hi->headset_state && gpio_get_value(hi->pdata.hpin_gpio)) {
			hpin_irq_type = IRQF_TRIGGER_HIGH;
			HS_LOG("Set HIGH level trigger");
		} else {
			HS_LOG("Set Low level trigger");
			hpin_irq_type = IRQF_TRIGGER_LOW;
		}
		set_irq_type(hi->hpin_irq, hpin_irq_type);
