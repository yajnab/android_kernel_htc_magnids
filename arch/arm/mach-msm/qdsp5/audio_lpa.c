...
static long audio_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == AUDIO_SET_VOLUME) {
		...
		cur_vol = audio->volume;
		new_vol = MSM_VOLUME_STEP * arg;
		new_vol /= MSM_VOLUME_FACTOR;
		if (new_vol > MSM_MAX_VOLUME)
			new_vol = MSM_MAX_VOLUME;

		for (i = 1; i < 4; i++) {
			audio->volume = cur_vol + ((new_vol - cur_vol) / 4 * i);
