...
static RAW_NOTIFIER_HEAD(earlysuspend_state_notifier_list);
static DEFINE_MUTEX(earlysuspend_state_notifier_lock);

int earlysuspend_state_notifier_register(struct notifier_block *nb)
{
	int ret;
	if (!nb)
		return -EINVAL;
	mutex_lock(&earlysuspend_state_notifier_lock);
	ret = raw_notifier_chain_register(&earlysuspend_state_notifier_list,
					  nb);
	mutex_unlock(&earlysuspend_state_notifier_lock);
	return ret;
}
EXPORT_SYMBOL(earlysuspend_state_notifier_register);

int earlysuspend_state_notifier_unregister(struct notifier_block *nb)
{
	int ret;
	if (!nb)
		return -EINVAL;
	mutex_lock(&earlysuspend_state_notifier_lock);
	ret = raw_notifier_chain_unregister(&earlysuspend_state_notifier_list,
					    nb);
	mutex_unlock(&earlysuspend_state_notifier_lock);
	return ret;
}
EXPORT_SYMBOL(earlysuspend_state_notifier_unregister);

static void earlysuspend_state_notify(uint32_t state, void *data)
{
	mutex_lock(&earlysuspend_state_notifier_lock);
	raw_notifier_call_chain(&earlysuspend_state_notifier_list,
				state, data);
	mutex_unlock(&earlysuspend_state_notifier_lock);
}
#endif
...
static void early_suspend(struct work_struct *work)
{
...
#ifdef CONFIG_EARLYSUSPEND_BOOST_CPU_SPEED
	earlysuspend_state_notify(EARLY_SUSPEND_HANDLE_START, NULL);
#endif
	if (debug_mask & DEBUG_SUSPEND)
	...
	mutex_unlock(&early_suspend_lock);
#ifdef CONFIG_EARLYSUSPEND_BOOST_CPU_SPEED
	earlysuspend_state_notify(EARLY_SUSPEND_HANDLE_END, NULL);
#endif
...
#ifdef CONFIG_EARLYSUSPEND_BOOST_CPU_SPEED
	earlysuspend_state_notify(LATE_RESUME_HANDLE_START, NULL);
#endif
	if (debug_mask & DEBUG_SUSPEND)
...
#ifdef CONFIG_EARLYSUSPEND_BOOST_CPU_SPEED
	earlysuspend_state_notify(LATE_RESUME_HANDLE_END, NULL);
#endif
